...
def emotion_detector(text_to_analyse):  # Define a function named emotion_detector that takes a string input (text_to_analyse)
    ...
    myobject = { "raw_document": { "text": text_to_analyse } }  # Create a dictionary with the text to be analysed
    header = {"grpc-metadata-mm-model-id": "emotion_aggregated-workflow_lang_en_stock"}  # Set the headers required for the API request

    response = requests.post(url, json = myobject, headers=header)  # Send a POST request to the API with the text and header
